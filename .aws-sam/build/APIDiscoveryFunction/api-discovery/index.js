...
module.exports = {
    getEndpoint: async function(path) {
        var position = null
        while (true) {
            var [id, nextPosition] = await this.findEndpoint(path, position);
    ...
    findEndpoint: async function(path, position) {
        var maxResults = '25'
        var params = {
            limit: maxResults,
        };
        ...
        return new Promise((resolve, reject) => {
            apigw.getRestApis(params, function(err, data) {
                ...
                var apiObject = data.items.find(out => out.name.substr(0, out.name.indexOf('-')) === path);
